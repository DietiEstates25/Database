...
/******************************************************************************
  * TYPE: constraint
  * NAME: bss_usr_fk_bss_usr
  *
  * DESC: recursive foreign key constraint associating each businesses user with 
          their superior
 ******************************************************************************/
ALTER TABLE tb_bss_usr
  ADD CONSTRAINT bss_usr_fk_bss_usr --*maybe change name to "recursive bla bla"?
...
ALTER TABLE tb_tmp_feature_sz
  ...
  FOREIGN KEY (id_estate) REFERENCES tb_tmp_estate(id)
  ON DELETE cascade
  ON UPDATE cascade;
