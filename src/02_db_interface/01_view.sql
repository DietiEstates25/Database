-- TODO: we really need to discuss this views
...
--     tb_feature_other.near_catering        AS "near_catering",
--     tb_feature_other.near_healthcare          AS "near_healthcare",
...
SELECT
    ...
    tb_address.st_number     AS "st_number"
FROM tb_estate
    JOIN tb_bss_usr
        ON tb_estate.id_bss_usr = tb_bss_usr.id_usr
    JOIN tb_agency
        ON tb_bss_usr.id_agency = tb_agency.id
    JOIN tb_address
        ON tb_estate.id_address = tb_address.id
...
FROM tb_usr_action
    JOIN tb_action_type
        ON tb_usr_action.id_action = tb_action_type.id
;
CREATE MATERIALIZED VIEW mv_estate_stats AS
SELECT 
    vw_typed_usr_action.id_estate        AS "id_estate",
    COUNT(vw_typed_usr_action.id_usr)       AS "n_actions",
